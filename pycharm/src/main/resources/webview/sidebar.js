...
(function () {
   ...
   * host (GotchiBrowserPanel.kt) at HTML-build time via the
  ...
  // Throttled to at most once per 30 s to avoid flooding the plugin host.
  var lastActivityPost = 0;
  document.addEventListener("mousemove", function () {
    var now = Date.now();
    if (now - lastActivityPost < 30000) { return; }
    lastActivityPost = now;
    window.__vscodeSendMessage(JSON.stringify({ command: "user_activity" }));
  });
