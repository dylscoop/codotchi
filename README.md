...
```bash
code --install-extension codotchi-2.20.4.vsix
```
...
1. Download `opencode-codotchi-2.20.4.zip` from the
   [Releases page](https://github.com/dylscoop/codotchi/releases).
2. Extract it and run the installer:

   ```bash
   # macOS / Linux
   unzip opencode-codotchi-2.20.4.zip && cd opencode-codotchi-2.18.0
   node bin/install.js --install
   ```

   ```powershell
   # Windows (PowerShell)
   Expand-Archive opencode-codotchi-2.20.4.zip; cd opencode-codotchi-2.18.0
   node bin/install.js --install
   ```
...
# produces pycharm/build/distributions/pycharm-codotchi-2.20.4.zip
```
