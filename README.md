...
1. Download `codotchi-2.5.0.vsix` from the Releases page.
...
```bash
code --install-extension codotchi-2.5.0.vsix
```

### JetBrains

1. Download `pycharm-codotchi-2.5.0.zip` from the Releases page.
   Do **not** unzip it.
...
1. Download `opencode-codotchi-2.5.0.zip` from the
   [Releases page](https://github.com/dylscoop/codotchi/releases).
2. Extract it and run the installer:
   ```bash
   # macOS / Linux
   unzip opencode-codotchi-2.5.0.zip && cd opencode-codotchi-2.5.0
   node bin/install.js --install
   ```
   ```powershell
   # Windows (PowerShell)
   Expand-Archive opencode-codotchi-2.5.0.zip; cd opencode-codotchi-2.5.0
   node bin/install.js --install
   ```
...
# produces pycharm/build/distributions/pycharm-codotchi-2.5.0.zip
```
...
Current release: **v2.5.0** — built by [dylscoop](https://github.com/dylscoop)
