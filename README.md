...
1. Download `codotchi-1.18.1.vsix` from the Releases page.
...
```bash
code --install-extension codotchi-1.18.1.vsix
```

### JetBrains

1. Download `pycharm-codotchi-1.18.1.zip` from the Releases page.
   Do **not** unzip it.
...
1. Download `opencode-codotchi-1.18.1.zip` from the
   [Releases page](https://github.com/dylscoop/codotchi/releases).
2. Extract it and run the installer:
   ```bash
   # macOS / Linux
   unzip opencode-codotchi-1.18.1.zip && cd opencode-codotchi-1.18.1
   node bin/install.js --install
   ```
   ```powershell
   # Windows (PowerShell)
   Expand-Archive opencode-codotchi-1.18.1.zip; cd opencode-codotchi-1.18.1
   node bin/install.js --install
   ```
...
# produces pycharm/build/distributions/pycharm-codotchi-1.18.1.zip
```
...
Current release: **v1.18.1** — built by [dylscoop](https://github.com/dylscoop)
