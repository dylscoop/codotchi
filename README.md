...
1. Download `vscode-gotchi-0.11.1.vsix` from the Releases page.
...
1. Download `pycharm-gotchi-0.11.1.zip` from the Releases page.
   Do **not** unzip it.
...
1. Download `opencode-codotchi-0.11.1.zip` from the
   [Releases page](https://github.com/dylscoop/codotchi/releases).
2. Extract it and run the installer:
   ```bash
   # macOS / Linux
   unzip opencode-codotchi-0.11.1.zip && cd opencode-codotchi-0.11.1
   node bin/install.js --install
   ```
   ```powershell
   # Windows (PowerShell)
   Expand-Archive opencode-codotchi-0.11.1.zip; cd opencode-codotchi-0.11.1
   node bin/install.js --install
   ```
...
npm install
npx vsce package
# produces vscode-gotchi-0.11.1.vsix
```
...
# produces pycharm/build/distributions/pycharm-gotchi-0.11.1.zip
```
...
Current release: **v0.11.1** — built by [dylscoop](https://github.com/dylscoop)
