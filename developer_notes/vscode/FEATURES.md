...
| OpenCode daily cost + token speech | `[x]` | Pet mentions daily USD cost and token count in speech bubble; three tiers: normal (< warn threshold), warning (>= warn), ALL CAPS shout (>= shout); thresholds configurable via `/codotchi warnthreshold` and `/codotchi shoutthreshold` (defaults: $30 / $50); persisted in `codotchi-daily.json` across sessions |
