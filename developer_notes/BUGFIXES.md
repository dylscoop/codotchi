...
**Fix:** Two changes to `git-workflow/SKILL.md`: (1) Added `node bin/install.js --install` explicitly to the "keep going" rule's stop-point list, making the gate unambiguous — OpenCode must stop and ask before running it. (2) Changed "move directly to the next todo item" / "move on" to "immediately execute the next todo item's command — do not output a status summary" in both the normal post-build rule (step 4) and the pasted-BUILD-SUCCESSFUL path (step 4) — removing the ambiguity that caused OpenCode to treat "continue" as "report status then pause" instead of "run the next command now". Also updated `PLUGIN_VER` in `install.js` from `1.2.27` to `1.14.19` to match the actual running plugin version.
